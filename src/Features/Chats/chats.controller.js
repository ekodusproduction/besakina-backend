...
export const createChatRoom = async function (req, res, next) {
    try {
        const [existingChatRow] = await db('chat_room')
            .where(function () {
        ...
        const [insertRows] = await db('chat_room').insert({ user_id_1: req.userId, user_id_2: req.body.user_id_2 });
