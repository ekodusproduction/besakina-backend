...
export const addAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    return sendResponse(res, "Hospitals added successfully", 201, { id: rows.insertId });
  } catch (error) {
    await connection.rollback()
...
export const getAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    const [query, values] = await selectJoinQuery('hospitals', [], { id: advertisementID, is_active: 1 });
...
export const filterAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    const filter = `SELECT id, title, price_registration, images, city, state FROM hospitals WHERE is_active = ? AND price_registration BETWEEN ? AND ?`;
    const [rows, fields] = await connection.query(filter, [true, minPrice || 0, maxPrice || 1000000000000]);

    if (rows.length === 0) {
      return sendError(res, "Hospitality not found for given filter", 404);
    }
...
export const addImage = async (req, res, next) => {
  ...
  const connection = await pool.getConnection();
  try {
    const [query1, values1] = await selectQuery('hospitals', ['images'], { id: advertisementID });
    ...
    const [query, values] = await updateQuery('hospitals', { "images": photosJson }, { id: advertisementID, is_active: 1 });
    const [rows, fields] = await connection.query(query, values);
    if (rows.length === 0) {
...
export const deleteImage = async (req, res, next) => {
  ...
  const connection = await pool.getConnection();
  try {
    const [results] = await selectQuery('hospitals', ['images'], { id: advertisementID });
    if (results.length === 0) {
