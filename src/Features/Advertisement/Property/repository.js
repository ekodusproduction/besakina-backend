...
const filterAdvertisement = async (query) => {
    ...
    try {
        ...
        return { error: false, data: { message: "property filter list", "data": { "property": data } }, statusCode: 200 };
    } catch (error) {
        logger.info(error);
        throw new ApplicationError(error, 500);
    } finally {
...
export const addImage = async (advertisementID, files, userId) => {
    ...
    try {
        ...
        return { error: false, data: { data: filePaths, message: "property image has been added.", statusCode: 200 } };
    } catch (error) {
