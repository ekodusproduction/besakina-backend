...
export const addImage = async (req, res, next) => {
  ...
  const connection = await pool.getConnection();
  try {
    const [query1, values1] = await selectQuery('vehicles', ['photos'], { id: advertisementID });
    ...
    const [query, values] = await updateQuery('vehicles', { "photos": photosJson }, { id: advertisementID, is_active: 1 });
    const [rows] = await connection.query(query, values);
    if (rows.length === 0) {
...
export const deleteImage = async (req, res, next) => {
  ...
  const connection = await pool.getConnection();
  try {
    const [results] = await selectQuery('vehicles', ['photos'], { id: advertisementID });
    if (results.length === 0) {
