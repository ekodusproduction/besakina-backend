...
export const addAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    if (rows.affectedRows === 0) {
      ...
      return sendError(res, "Error adding Vehicles", 400);
    }
    await connection.commit();
    return sendResponse(res, "Vehicle added successfully", 201, { id: rows.insertId });
  } catch (error) {
    await connection.rollback()
    next(error)
  } finally {
...
export const getAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    const [query, values] = await selectJoinQuery('vehicles', ['vehicles.*', 'users.*'], 'users', 'vehicles.user_id = users.id', { 'vehicles.id': advertisementID, 'vehicles.is_active': 1 });
    ...
    return sendResponse(res, "Vehicles fetched successfully", 200, { Vehicles: rows[0] });
  } catch (error) {
    return sendError(res, error.message || "Error fetching Vehicles", 500);
  } finally {
    if (connection) {
      connection.release();
    }
...
export const getListAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    return sendResponse(res, "Advertisements fetched successfully", 200, { Vehicles: rows });
  } catch (error) {
...
export const filterAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    return sendResponse(res, "Vehicles fetched successfully", 200, { advertisements: rows });
  } catch (error) {
    return sendError(res, error.message || "Error fetching Vehicles", 500);
  } finally {
    if (connection) {
...
export const updateAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    const [rows, fields] = await connection.query(query, values);
    if (rows.length === 0) {
      return sendError(res, "Vehicles not updated. No matching Vehicles found for the provided ID.", 404);
    }
...
export const deleteAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    const [query, values] = await updateQuery('vehicles', { "is_active": 0 }, { id: advertisementID, is_active: 1 });
    const [rows, fields] = await connection.query(query, values);
    if (rows.changedRows === 0) {
      return sendError(res, "Vehicles not deleted. No matching Vehicles found for the provided ID.", 404);
    }
...
export const addImage = async (req, res, next) => {
  ...
  const connection = await pool.getConnection();
  try {
    const [query1, values1] = await selectQuery('vehicles', ['images'], { id: advertisementID });
    ...
    const [query, values] = await updateQuery('vehicles', { "images": photosJson }, { id: advertisementID, is_active: 1 });
    const [rows, fields] = await connection.query(query, values);
    if (rows.length === 0) {
      return sendError(res, "Failed to add images to the Vehicles.", 404);
    }
    return sendResponse(res, "Images added successfully to the Vehicles", 200, { advertisements: rows });
  } catch (error) {
    await deleteFiles(files)
    await connection.rollback();
    return sendError(res, error.message || "Error adding images to the Vehicles", 500);
  } finally {
    if (connection) {
      connection.release();
    }
...
export const deleteImage = async (req, res, next) => {
  ...
  const connection = await pool.getConnection();
  try {
    const [results] = await selectQuery('vehicles', ['images'], { id: advertisementID });
    if (results.length === 0) {
      return sendError(res, "Vehicles not found.", null, 404);
    }
    ...
    const [rows] = await connection.query(query, values);
    if (rows.length === 0) {
      return sendError(res, "Failed to delete images of the Vehicles.", null, 404);
    }
    await deleteFiles(images)
    return sendResponse(res, "Images deleted successfully of the Vehicles", { advertisements: rows }, 200);
  } catch (error) {
    await connection.rollback();
    return sendError(res, error.message || "Error deleting images to the Vehicles", null, 500);
  } finally {
    if (connection) {
      connection.release();
    }
...
export const listUserAdvertisement = async (req, res, next) => {
  ...
  let connection = await pool.getConnection();
  try {
    const [query, values] = await selectQuery('vehicles', [], { user_id: user_id });
    ...
    return sendError(res, error.message || "Error deleting images to the Vehicles", 500);
  } finally {
    if (connection) {
      connection.release();
    }
