...
const filterAdvertisement = async (query) => {
    ...
    try {
        ...
        return { error: false, data: { message: "hospitality filter list", "data": { "hospitality": data } } };
    } catch (error) {
        logger.info(error);
        throw new ApplicationError(error, 500);
    } finally {
...
export const addImage = async (advertisementID, files, userId) => {
    ...
    try {
        ...
        return { error: false, data: { data: filePaths, message: "hospitality image has been added.", statusCode: 200 } };
    } catch (error) {
