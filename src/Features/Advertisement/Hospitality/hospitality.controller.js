...
export const addAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    return sendResponse(res, "Hospitality added successfully", 201, { id: rows.insertId });
  } catch (error) {
    await connection.rollback()
    next(error)
  } finally {
...
export const getAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    const [query, values] = await selectJoinQuery('hospitality', ['*'], 'users', 'hospitality.user_id = users.id', { id: advertisementID, is_active: 1 });
...
export const updateAdvertisement = async (req, res, next) => {
  ...
  try {
    ...
    const [query, values] = await updateQuery('hospitality', filter, { id: advertisementID });
    const [rows, fields] = await connection.query(query, values);
    if (rows.length === 0) {
...
export const addImage = async (req, res, next) => {
  ...
  try {
    ...
    const [query, values] = await updateQuery('hospitality', { "images": photosJson }, { id: advertisementID, is_active: 1 });
    const [rows, fields] = await connection.query(query, values);
    if (rows.length === 0) {
...
export const deleteImage = async (req, res, next) => {
  ...
  const connection = await pool.getConnection();
  try {
    const [results] = await selectQuery('hospitality', ['images'], { id: advertisementID });
    if (results.length === 0) {
...
export const listUserAdvertisement = async (req, res, next) => {
  ...
  let connection = await pool.getConnection();
  try {
    const [query, values] = await selectQuery('hospitality', [], { user_id: user_id });
