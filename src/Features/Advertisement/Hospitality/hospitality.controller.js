...
export const getAdvertisement = async (req, res, next) => {
  try {
    ...
    if (!advertisement) {
      return sendError(res, "hospitalality not found", 404);
    }
    return sendResponse(res, "hospitalality fetched successfully", 200, { advertisement });
  } catch (error) {
    logger.info(error)
    next(error);
  }
...
export const getListAdvertisement = async (req, res, next) => {
  try {
    ...
    return sendResponse(res, "hospitalality fetched successfully", 200, { advertisements });
  } catch (error) {
    logger.info(error)
    next(error);
  }
...
export const filterAdvertisement = async (req, res, next) => {
  try {
    ...
    if (advertisements.length === 0) {
      return sendError(res, "hospitalality not found for given filter", 404);
    }
