...
export const getListAdvertisement = async () => {
    try {
        ...
        const doctors = await Doctor.find({ is_active: true }).sort({ created_at: -1 }).skip(offset)
            .limit(limit);
...
const filterAdvertisement = async (query) => {
    try {
        ...
        return { error: false, data: { message: "Doctor filter list", statusCode: 200, data: { doctors: result } } };
    } catch (error) {
        logger.info(error);
        throw new ApplicationError(error, 500);
    }
