...
export const getListAdvertisement = async (req, res, next) => {
  try {
    ...
    return sendResponse(res, "Doctors fetched successfully", 200, { "doctors": advertisements });
  } catch (error) {
    logger.info(error)
    next(error);
  }
...
export const filterAdvertisement = async (req, res, next) => {
  try {
    ...
    return sendResponse(res, advertisements.message, 200, { "doctors": advertisements.data });
  } catch (error) {
    logger.info(error)
    next(error);
  }
