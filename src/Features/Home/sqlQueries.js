...
UNION ALL
(
    SELECT id, title, price, created_at, images, city, state, 'hospitality' AS category FROM hospitality WHERE MATCH (title, name, type, description, street, city, locality, pincode) AGAINST ? ORDER BY created_at DESC LIMIT 10
)
UNION ALL
(
    SELECT id, title, price, created_at, images, city, state, 'education' AS category FROM education WHERE MATCH (title, domain, institution_name, description, street, city, locality, pincode) AGAINST ? ORDER BY created_at DESC LIMIT 10
)
UNION ALL
(
    SELECT id, title, price_per_visit, created_at, images, city, state, 'doctors' AS category FROM doctors WHERE MATCH (title, expertise, description, street, city, locality, pincode) AGAINST ? ORDER BY created_at DESC LIMIT 10
)
UNION ALL
(
    SELECT id, title, price_registration, created_at, images, city, state, 'hospitals' AS category FROM hospitals WHERE MATCH (title, name, type, description, street, city, locality, pincode) AGAINST ? ORDER BY created_at DESC LIMIT 10
)
ORDER BY FIELD(category, 'property', 'vehicles', 'hospitality', 'education', 'doctors', 'hospitals'), created_at DESC
`