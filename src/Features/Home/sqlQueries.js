...
export const searchAdd = `(
    SELECT id, title, price, created_at, images,city, state, 'property' AS category
    FROM property
    WHERE is_active = 1 AND MATCH(title, type, street, city, house_no, pincode) AGAINST ?
    ORDER BY created_at DESC
    LIMIT 10
)
UNION ALL
(
    SELECT id, title, price, created_at, images,city, state, 'vehicles' AS category
    FROM vehicles
    WHERE is_active = 1 AND MATCH(title, brand, type, description,street, city, locality, pincode) AGAINST ?
    ORDER BY created_at DESC
    LIMIT 10
)
UNION ALL
(
    SELECT id, title, price, created_at, images,city, state, 'hospitality' AS category
    FROM hospitality
    WHERE is_active = 1 AND MATCH(title, name, type, description,street, city, locality, pincode) AGAINST ?
    ORDER BY created_at DESC
    LIMIT 10
)
UNION ALL
(
    SELECT id, title, price, created_at, images,city, state, 'education' AS category
    FROM education
    WHERE is_active = 1 AND MATCH(title, domain, institution_name, description,street, city, locality, pincode) AGAINST ?
    ORDER BY created_at DESC
    LIMIT 10
)
UNION ALL
(
    SELECT id, title, price_per_visit, created_at, images,city, state, 'doctors' AS category
    FROM doctors
    WHERE is_active = 1 AND MATCH(title, expertise, description,street, city, locality, pincode) AGAINST ?
    ORDER BY created_at DESC
    LIMIT 10
)
UNION ALL
